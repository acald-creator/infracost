...
func (r *LogAnalyticsWorkspace) BuildResource() *schema.Resource {
	...
	if r.SKU == skuPerGB2018 {
		costComponents = append(costComponents, r.logDataIngestion(), r.sentinelDataIngestion())
	}

	if r.SKU == skuCapacityReservation && r.ReservationCapacityInGBPerDay > 0 {
		costComponents = append(costComponents, r.logDataIngestionFromCapacityReservation(), r.sentinelDataIngestionFromCapacityReservation())
	}
